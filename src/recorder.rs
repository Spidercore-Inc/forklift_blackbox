...
// 영상 인코딩 파이프라인 제작 함수
fn build_encoder_pipeline(target_folder: &str, timestamp: &str) -> gstreamer::Pipeline {
    // 파이프라인 생성
    let pipeline = gstreamer::parse::launch(&(ENCODER.to_owned() + target_folder + "/" + timestamp + ".mp4")).unwrap_or_else(|e| errorlog("Failed to parse encoder pipeline", Some(e)));
...
fn buffer_to_mp4(
    ...
    */
    ...
    info!("10/1 FPS Video has been saved");
...
pub fn encoder_thread(
    ...
    let my_last_state = RecorderState::Record(Device::SD);
    let folder_name = format!("camera_{}", thread_num);
        ...
        */
        ...
        match my_last_state {
            RecorderState::Record(device) => {
                ...
                let target_folder = format!("{}/{}", save_dir, &folder_name);
                //info!("SubThread: Target folder is {}", &folder_name);

                // 버퍼를 MP4 파일로 내보낸다.
                buffer_to_mp4(frame_buffer_copy, &target_folder, &timestamp);
